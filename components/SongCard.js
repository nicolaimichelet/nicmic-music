...
export default function SongCard({
    ...
    return (
        <div className="flex flex-col">
            <Image
                className="w-[16rem] h-[16rem] md:w-[20rem] md:h-[20rem] lg:w-[24rem] lg:h-[24rem] object-contain rounded-lg transition-transform duration-500 group-hover:scale-110 group-hover:opacity-100 cursor-overflow-hidden transition-shadow hover:shadow-xl"
                src={source}
                alt={alt}
                width={3000}
                height={3000}
            ...
            <div
                className={`song-card ${
                    isActive ? 'activeSongCard' : 'inactive'
                }`}
            >
                {isActive ? (
                    <div>
